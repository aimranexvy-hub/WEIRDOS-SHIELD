  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 font-sans">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
        <div className="absolute top-[-10%] left-[-10%] w-[50%] h-[50%] bg-purple-500/[0.02] blur-[150px] rounded-full" />
      </div>
      ...
      <nav className="fixed top-0 w-full z-50 border-b border-white/[0.03] bg-[#020617]/80 backdrop-blur-3xl px-6 h-20 flex items-center justify-between">
        <div className="flex items-center gap-8">
          <Link href="/" className="flex items-center gap-4 group">
            <div className="h-8 w-8 rounded-lg bg-purple-600 flex items-center justify-center mr-3">
              <Bot className="h-5 w-5 text-white" />
            </div>
            <span className="text-xl font-bold text-white font-outfit uppercase tracking-tighter hidden md:block">Weirdos Shield Docs</span>
          </Link>
          
          <div className="hidden lg:flex items-center w-80 relative group">
            <SearchIcon className="absolute left-4 h-4 w-4 text-slate-500 group-focus-within:text-purple-500 transition-colors" />
            <input 
              type="text" 
      ...
      <div className="max-w-7xl mx-auto flex pt-20">
                  ...
                  {section.items.map((item) => (
                    <button
                      ...
                      className={cn(
                        ...
                        activeTab === item.name 
                          ? "bg-purple-500/10 border border-purple-500/20 shadow-[0_0_20px_rgba(239,68,68,0.05)]" 
                          : "hover:bg-white/[0.02] border border-transparent"
                      )}
                    >
                      <span className={cn("text-sm font-bold", activeTab === item.name ? "text-purple-500" : "text-slate-300")}>{item.name}</span>
        ...
        <main className="flex-1 p-8 lg:p-16 relative z-10 max-w-4xl">
          ...
          <h1 className="text-6xl font-bold text-white font-outfit tracking-tighter uppercase mb-8 italic">
            {activeTab}<span className="text-purple-500 not-italic">.</span>
          </h1>

          <div className="prose prose-invert max-w-none">
             <p className="text-lg text-slate-400 mb-12 leading-relaxed">
               Welcome to the {activeTab} section of the Weirdos Shield Engine documentation. Our engine is designed for communities that demand absolute performance and cinematic management tools.
             </p>
             ...
             <div className="p-8 rounded-[40px] bg-purple-500/[0.02] border border-purple-500/10 relative overflow-hidden">
                ...
                <p className="text-slate-500 font-bold leading-relaxed mb-8">
                  The Weirdos Shield Engine utilizes a decentralized event stream processing model. When a Discord event is received, it is instantly routed to the nearest edge cluster.
                </p>
                <div className="bg-black/40 p-6 rounded-2xl border border-white/5 font-mono text-sm text-purple-500 mb-8">
                  $ Weirdos Shield initialize --cluster-shard [neural_07] --mode enterprise
                </div>
             </div>
          </div>
          ...
          <div className="mt-20 pt-12 border-t border-white/5 flex items-center justify-between">
             ...
             <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase text-purple-500 tracking-[0.2em]">Live Stream Active</span>
             </div>
          </div>
        </main>
      </div>
